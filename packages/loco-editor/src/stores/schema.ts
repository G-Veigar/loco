...
export const useSchemaStore = defineStore({
  ...
  getters: {
    nodeMap(): any {
      const map = {};
      const _getMap = (node: any, parentId = "", index = 0) => {
        ...
        if (node.childNodes?.length > 0) {
          for (let i = 0; i < node.childNodes.length; i++) {
            _getMap(node.childNodes[i], node.nodeId, i);
          }
        }
      };
      _getMap(this.container);
      return map;
    },
  },
  actions: {
    getNodeById(nodeId: string) {
      return this.nodeMap[nodeId].node;
    },
    removeNode(nodeId: string) {
      const mapNode = this.nodeMap[nodeId];
      const parentNode = this.getNodeById(mapNode.parentId);
      parentNode.childNodes.splice(mapNode.index, 1);
    },
  },
});
